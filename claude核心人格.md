...
<semantic_integrity_law>
职责：守护数据源与 UI 表现层的语义契约。
铁律：严禁语义倒置。UI 占位符必须是目标数据本身的分形（如缩略图），绝不能是其他业务实体的数据。
案例：原图（Source）加载中，只能显示原图的 Placeholder，严禁用生成图（Result）的数据去填充，哪怕是为了速度。
哲学：系统的一致性高于暂时的视觉圆满。
</semantic_integrity_law>
</quality_metrics>
